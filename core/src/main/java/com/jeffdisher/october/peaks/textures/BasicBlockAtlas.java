...
public class BasicBlockAtlas
{
	...
	public BasicBlockAtlas(Block[] blocksIncluded, RawTextureAtlas blockTextures, boolean[] nonOpaqueVector)
	{
	...
	{
		short index = _itemToBlockMap[value];
		int rawIndex = _mapToRawIndex(index, Variant.TOP);
	...
	{
		short index = _itemToBlockMap[value];
		int rawIndex = _mapToRawIndex(index, Variant.BOTTOM);
		return _blockTextures.baseOfTexture(rawIndex);
	}
	...
	{
		short index = _itemToBlockMap[value];
		int rawIndex = _mapToRawIndex(index, Variant.SIDE);
		return _blockTextures.baseOfTexture(rawIndex);
	}
	...
	private static int _mapToRawIndex(short index, Variant variant)
	{
		return (index * Variant.values().length) + variant.ordinal();
	}
