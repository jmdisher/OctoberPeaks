...
public class ViewCraftingPanel implements IView<List<ItemTuple<CraftDescription>>>
{
	...
	)
	{
		...
		ComplexItemView.IBindOptions<CraftDescription> options = new ComplexItemView.IBindOptions<>()
		{
			@Override
			public int getOutlineTexture(ItemTuple<CraftDescription> context)
			{
				...
				for (CraftDescription.ItemRequirement items : context.context().input())
				{
			...
			@Override
			public void hoverRender(Point cursor, ItemTuple<CraftDescription> context)
			{
				// This hover is pretty complicated since we draw the name an inputs.
				CraftDescription craft = context.context();
			...
			@Override
			public void hoverAction(ItemTuple<CraftDescription> context)
			{
				craftHoverOverItem.accept(context.context());
			}
		};
