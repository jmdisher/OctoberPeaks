...
public class AudioManager
{
		...
		for (PartialEntity other : _otherEntities.values())
		{
			...
			if (null != soundToPlay)
			{
				...
				if (distanceSquared <= AUDIO_RANGE_SQUARED)
				{
					...
					{
						_playSound(soundToPlay, otherLocation, distanceSquared);
					}
				}
			}
		}
	...
	private void _playSound(Sound soundToPlay, AbsoluteLocation otherLocation, int distanceSquared)
	{
		float floatSquaredDistance = (float)distanceSquared;
	...
	private void _playSoundIfInRange(AbsoluteLocation location, Sound soundToPlay)
	{
		...
		if (distanceSquared <= AUDIO_RANGE_SQUARED)
		{
			_playSound(soundToPlay, location, distanceSquared);
		}
	}
